...
// NAYA: login/register ke time device create/update karta hai, aur
// USI device pe refresh token save karta hai (User pe nahi)
async function upsertDevice(req, userId, refreshToken) {
  ...
  // deviceId zaroori hai — bina iske hum devices track nahi kar sakte
...
exports.register = asyncHandler(async (req, res) => {
  ...
  // FIX: refresh token ab User pe nahi, is device pe save hoga
...
exports.login = asyncHandler(async (req, res) => {
  ...
  // FIX: har device ka apna refresh token save hoga, ek-doosre ko overwrite nahi karega
  await upsertDevice(req, user._id, refreshToken);
...
exports.refreshToken = asyncHandler(async (req, res) => {
  ...
  // FIX: ab User.refreshToken ki jagah Device.refreshToken se match karo —
  // isse pata chalta hai KIS device ka token hai, aur agar wo device
  // pehle se remove/logout ho chuka hai, to refresh fail ho jayega
  const device = await Device.findOne({ user: decoded.id, refreshToken });
  if (!device) {
  ...
  // Device ko "abhi active hai" mark kar do
...
exports.logout = asyncHandler(async (req, res) => {
  ...
  // FIX: pehle User.refreshToken clear hota tha, ab is specific device
  // ka Device.refreshToken clear/delete karo — baaki devices logged-in rahenge
  if (deviceId) {
    await Device.deleteOne({ user: req.user._id, deviceId });
  }
